...
<ParamField body="params" type="object" required>
  ...
  <Expandable title="params オブジェクトのプロパティ">
    ...
    <ParamField body="type" type="string" required>
      参照モード、画像/動画パラメーターの使用方法を決定します

      * `all_reference` — インテリジェント参照モード：`image_url` はキャラクター/スタイル参照画像（最大9枚）、`video_url` は参照動画（最大3つ）として使用
      * `first_last_frame` — 最初と最後のフレームモード：`image_url` が最初のフレーム（1枚のみ）、`end_image_url` が最後のフレーム、`video_url` は非対応
    ...
    <ParamField body="image_url" type="string[]">
      ...
      * `all_reference` モード：最大9枚、各最大50MB、最小辺長300px
      * `first_last_frame` モード：1枚のみ（最初のフレーム）、最大10MB、最小辺長300px
    </ParamField>

    <ParamField body="video_url" type="string[]">
      参照動画のURL

      `all_reference` モードのみ対応。最大3つ。対応形式は `mp4`、`mov`。単一動画の長さは2〜15秒、すべての参照動画の合計長さは15秒以内。単一動画の最大サイズは50MB。フレームレートは24〜60FPS。解像度は `480p`、`720p` に対応。アスペクト比は0.4〜2.5。幅と高さの範囲は300〜6000px。総ピクセル数は409600から927408の間である必要があります。

      <Warning>`all_reference` モードのみが有効で、`type` が `first_last_frame` の場合はこのパラメーターを渡さないでください</Warning>
    </ParamField>
    ...
    <ParamField body="video_length" type="string" default="5">
      ...
      対応長さ範囲：4〜15秒、デフォルトは5秒
    </ParamField>

    <ParamField body="generate_audio" type="boolean" default="true">
      音声を生成するか、デフォルトは `true`
    </ParamField>
  </Expandable>
</ParamField>
