...
description: |
    ...
    - 生成されたビデオリンクの有効期限は24時間です。お早めに保存してください
...
<ParamField path="video_id" type="string" required>
  ...
  これは以前にビデオ生成 API から返されたタスクIDです
</ParamField>

## Authorizations

<ParamField header="Authorization" type="string" required>
  すべての API エンドポイントは Bearer Token 認証が必要です
...
<ParamField body="model" type="string" default="sora-2" required>
  ...
  * `sora-2` - 標準バージョン
  * `sora-2-pro` - プロフェッショナルバージョン、より長い時間に対応
...
<ResponseField name="object" type="string">
  オブジェクトタイプ、固定値 `video.remix.task`
</ResponseField>

<ResponseField name="progress" type="integer">
  タスク完了の進捗パーセンテージ (0-100)
</ResponseField>
...
<ResponseExample>
  ...
  ```json 403
  {
    "error": {
      "code": 403,
      "message": "アクセス拒否。このリソースへの権限がありません",
  ...
  ```json 429
  {
    "error": {
      "code": 429,
      "message": "レートリミット超過。後でもう一度お試しください",
  ...
  ```json 502
  {
    "error": {
      "code": 502,
      "message": "バッドゲートウェイ。サーバーは一時的に利用できません",
      "type": "bad_gateway"
    }
  }
  ```
