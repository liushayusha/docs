...
<ParamField body="model" type="string" required>
  モデル名。以下のチャネルバージョンをサポート（チャネルはサービスの安定性にのみ影響し、出力品質には影響しません）：

  * `"nano_banana_pro"` - デフォルト
  * `"nano_banana_pro_base"` - ベーシックバージョン
  * `"nano_banana_pro_plus"` - 拡張バージョン
  * `"nano_banana_pro_advanced"` - アドバンストバージョン
  * `"nano_banana_pro_max"` - フラッグシップバージョン
</ParamField>
    ...
    <ParamField body="image_url" type="string[]">
      ...
      * 以下の2形式をサポート：
        ...
        2. Base64エンコード形式
           ...
           * ⚠️ 注意：`data:image/jpeg;base64,`のプレフィックスを含める必要があり、base64画像のサイズは10MBを超えないこと
    </ParamField>
