---
title: 'Nano Banana V2 画像生成'
description: |
    - 非同期処理モードで、タスクIDを返し、後続のクエリに使用
    - テキストから画像生成、画像変換、画像編集など多様な生成モードに対応
...
<ParamField body="model" type="string" required>
  モデル名。以下のチャンネルバージョンをサポートします（チャンネルはサービスの安定性にのみ影響し、出力の品質には影響しません）：

  * `"nano_banana_v2"` - デフォルト
  * `"nano_banana_v2_base"` - 基本版
  * `"nano_banana_v2_plus"` - 拡張版
  * `"nano_banana_v2_max"` - フラッグシップ版
</ParamField>
    ...
    <ParamField body="image_url" type="string[]">
      ...
      * 以下の2つの形式をサポート：
        ...
        2. Base64エンコード形式
           ...
           * ⚠️ 注意：`data:image/jpeg;base64,`のプレフィックス部分を含める必要があります。base64画像のサイズは10MBを超えないこと
    </ParamField>
...
<ParamField body="out_task_id" type="string" required>
  ...
  ユーザーがカスタムで指定するタスク識別子。必須
</ParamField>

## Response

<ResponseField name="out_task_id" type="string">
  発起側タスクID、結果のクエリに使用可能
</ResponseField>
...
```json 433
  ...
  ```json 500
  {
    "error": {
      "code": 500,
      "message": "サーバ内部エラーが発生しました。しばらくしてから再試行してください"
    }
  }
  ```
</ResponseExample>