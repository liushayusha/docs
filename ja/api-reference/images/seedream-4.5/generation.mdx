...
description: |
    ...
    - 生成された画像のリンクは有効期限が24時間のため、早めに保存してください
...
<ParamField body="resolution" type="string" default="2K">
  ...
  **解像度の参考サイズ：**
  ...
  <Expandable title="完全なピクセルマッピングテーブル">
    **2K 解像度**
...
<ParamField body="n" type="integer">
  ...
  取扱範囲：1-15（最小 1 枚、最大 15 枚）

  デフォルト値：1

  **注意：**

  * 実際に生成される画像枚数はこのパラメータだけでなく、入力された参考画像の枚数にも影響されます。参考画像の枚数 + 最終生成画像枚数 ≤ 15 枚
  * **数値のみ入力してください（例：`1`）。引用符を付けるとエラーとなります**

  生成枚数に基づいて事前課金されます
</ParamField>

<ParamField body="image_urls" type="array">
  参考画像の URL のリスト。画像から画像生成または画像編集に使用します
...
<ParamField body="optimize_prompt_options.mode" type="string" default="standard">
  ...
  * `standard`：標準モード。生成内容の品質が高く、処理時間は長め
  * `fast`：高速モード。生成時間は短いが品質は普通

  デフォルト値：`standard`
</ParamField>

<ParamField body="sequential_image_generation" type="string" default="disabled">
  シーケンシャル画像生成モード（豆包の専用機能）

  複数画像を生成するかどうかを制御します：

  * `disabled`：シーケンシャルモードを無効化。参考画像が複数あっても画像は 1 枚のみ生成されます（デフォルト）
  * `auto`：シーケンシャルモードを有効化し、複数画像の生成が可能になります

  **使い方の説明：**

  * ✅ `image_urls` が必須 - 少なくとも 1 枚の参考画像が必要です
  * ✅ `n: 3` を設定するか、 `sequential_image_generation: "auto"` と `max_images: 3` を設定します
  * ✅ これで参考画像を基に異なる 3 枚の画像を生成します
  * ⚠️ `n > 1` の場合は自動的に `auto` に設定されます

  **制限：**

  * 参考画像なしの完全テキスト生成からは複数画像は生成できません。これは豆包 API の制約です
</ParamField>
...
<ParamField body="sequential_image_generation_options" type="object">
  ...
  * `max_images` (integer): 生成する画像の枚数を指定。範囲：1-15
...
<ResponseExample>
  ...
  ```json 502
  {
    "error": {
      "code": 502,
      "message": "ゲートウェイエラーです。サーバーは一時的に利用できません",
      "type": "bad_gateway"
    }
  }
  ```
