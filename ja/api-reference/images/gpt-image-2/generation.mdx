...
description: |
    ...
    - OpenAI Images 互換プロトコルに基づき、テキスト画像生成 / 画像から画像生成をサポート
    - 13種類の画像比率に対応し、aspect_ratio フィールドで指定可能
    - 参考画像は最大16枚までで、URLとbase64の混在が可能
    - 提出されたプロンプトはプラットフォームの敏感語句 / セキュリティチェックを通過し、違反内容は即時拒否
...
<ParamField body="params" type="object" required>
  ...
  <Expandable title="params オブジェクトのプロパティ">
    <ParamField body="prompt" type="string" required>
      ...
      * 日本語と英語に対応。詳細な記述を推奨。
      * 提出前にプラットフォームの不適切ワード/安全審査を通過し、違反内容があればエラーを返します。
    </ParamField>

    <ParamField body="image_url" type="string[]">
      リファレンス画像配列。渡すと画像変換モードになります。
      ...
      * 次の2つのフォーマットをサポート：
        ...
        2. Base64エンコード形式
           ...
           * ⚠️ 注意：`data:image/jpeg;base64,`のプレフィックス部分を必ず含むこと
    </ParamField>
    ...
    <ParamField body="aspect_ratio" type="string" default="1:1">
      ...
      * `21:9` / `9:21` - 超ワイド横 / 超縦長
      ...
      <Warning>
        数値のピクセル指定（例：`1024x1024`）はサポートせず、エラーになります。
      </Warning>
    </ParamField>
  </Expandable>
</ParamField>
