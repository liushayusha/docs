---
title: 'Seedream V5 Pro 画像生成'
description: |
    - 非同期処理モード、タスクIDを返し後続のクエリに使用
    - 高精度な生画像シーン向け、より正確な位置と要素の制御
    - テキスト生成画像、単一/複数の参照画像生成、カスタム出力サイズに対応
  ...
  <Expandable title="params オブジェクトのプロパティ">
    ...
    <ParamField body="image_url" type="string[]">
      ...
      * 以下の2種類の形式をサポートしています：
        ...
        2. Base64エンコード形式
           ...
           * ⚠️ 注意：`data:image/jpeg;base64,`のプレフィックス部分を含む必要があり、base64画像のサイズは10MBを超えません
    </ParamField>
    ...
    <ParamField body="aspect_ratio" type="string">
      ...
      * `21:9` - 超ワイドスクリーン
      ...
      ⚠️ `size`パラメーターと排他：`size`でカスタムサイズを渡した場合、`aspect_ratio`は無効になります
    </ParamField>
    ...
    <ParamField body="size" type="string">
      カスタム出力サイズ、フォーマットは「幅x高さ」（ピクセル値）、例：`2048x1024`

      * 幅と高さはそれぞれ16の倍数であること
      * 総ピクセル数（幅×高さ）の範囲：[921600, 4194304]（つまり1280x720 ～ 2048x2048）
      * アスペクト比の範囲：[1/16, 16]
      * `aspect_ratio` / `resolution`のプリセット方式と排他で、渡された場合はそのピクセル値で正確に画像を生成します
    </ParamField>
  </Expandable>
</ParamField>
...
<RequestExample>
  ...
  curl --request POST \
    ...
    --data '{
      ...
      "params": {
        "prompt": "一匹のオレンジ猫が窓辺に座っていて、窓の外は東京の夜景、サイバーパンクスタイル",
        "aspect_ratio": "16:9",
        "resolution": "2K"
      },
...
```json 433
  ...
  ```json 500
  {
    "error": {
      "code": 500,
      "message": "サーバ内部エラーです。後でもう一度お試しください"
    }
  }
  ```
</ResponseExample>