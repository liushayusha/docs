...
description: |
    - 非同期処理モードで、後続の確認に使用するタスクIDを返します
    - 画像の背景をワンクリックで除去し、きれいな被写体切り抜きを生成します
  ...
  <Expandable title="params オブジェクトの Properties">
    <ParamField body="image_url" type="string[]" required>
      ...
      * 以下の2つの形式をサポート:
      ...
      <Warning>
        URL はインターネットから直接アクセス可能である必要があり、ログイン、Cookie、または一時的なページ認証を要求してはいけません。
      </Warning>
    </ParamField>
  </Expandable>
</ParamField>
...
<ParamField body="out_task_id" type="string" required>
  呼び出し側がカスタマイズするタスク ID。冪等な送信と結果の取得に使用します。
...
送信が成功した後、同じ `out_task_id` を使用して呼び出します:
