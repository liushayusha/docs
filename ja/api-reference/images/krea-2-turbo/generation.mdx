...
description: |
    ...
    - 超高速なテキストから画像生成で、速度と映画品質の画質向けに最適化されています
    - 1Kと2Kの解像度、11種類のアスペクト比をサポート；テキストから画像生成のみサポートします
...
<ParamField header="Authorization" type="string" required>
  すべてのエンドポイントはBearer Tokenを使用して認証する必要があります
...
<ParamField body="params" type="object" required>
  ...
  <Expandable title="params オブジェクトのプロパティ">
    ...
    <ParamField body="seed" type="integer">
      オプション。再現可能な生成のための乱数シード

      範囲は 0-1048576。省略するか 0 を指定するとランダムに生成されます
    </ParamField>
  </Expandable>
</ParamField>
...
<ResponseField name="out_task_id" type="string">
  リクエスト元のタスクID。結果の検索に使用できます
</ResponseField>

<ResponseField name="status" type="string">
  初期ステータス。固定で `"pending"` です
</ResponseField>
...
<RequestExample>
  ...
  curl --request POST \
    ...
    --data '{
      "model": "krea_2_turbo",
      "params": {
        "prompt": "窓辺に座るオレンジ色の猫。窓の外は東京の夜景で、サイバーパンク風",
        "aspect_ratio": "16:9",
        "resolution": "2K"
      },
