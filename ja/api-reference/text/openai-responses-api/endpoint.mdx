...
description: |
    ...
    - 主に Codex CLI や AI SDK などのツールで使用
...
<ParamField body="model" type="string" required>
  使用するモデルの名前です。例：`claude-sonnet-4-5-20250929`、`gpt-4o`。

  完全なモデルリストは `GET /api/v1/models` で取得可能です。
</ParamField>

<ParamField body="input" type="string | array" required>
  会話の入力内容で、2つの形式に対応しています：

  - **文字列**：ユーザーメッセージのテキストを直接渡す
  - **メッセージ配列**：複数のメッセージを含み、複数ターンの会話に対応

  <Expandable title="メッセージ配列の形式">
    各メッセージはオブジェクトで、以下のフィールドを含みます：
    ...
    <ParamField body="content" type="string | array" required>
      メッセージ内容はテキストの文字列、またはコンテンツブロックの配列（画像などのマルチモーダルコンテンツ対応）で指定可能：
...
<ParamField body="instructions" type="string">
  システムプロンプト。モデルの行動基準、役割、コンテキストを設定します。

  メッセージ配列中の `role: "system"` に相当します。
</ParamField>

<ParamField body="stream" type="boolean" default="true">
  ストリーミング出力を有効にするかどうか。

  - `true`（デフォルト）：SSEイベントストリームでトークン単位に返し、リアルタイム表示に適しています
  - `false`：完全なレスポンスを待って一括で返し、一括処理に適しています
</ParamField>

<ParamField body="max_output_tokens" type="integer">
  生成するレスポンスの最大トークン数です。制限を超えた場合、`status` は `"incomplete"`、`incomplete_details.reason` は `"max_output_tokens"` になります。
</ParamField>

<ParamField body="temperature" type="number">
  サンプリング温度、範囲は `0`～`2`。値が高いほど出力はランダムに、低いほど決定的になります。`temperature` と `top_p` を同時に変更することは推奨されません。
</ParamField>

<ParamField body="top_p" type="number">
  nucleusサンプリングの確率閾値、範囲は `0`～`1`。モデルは累積確率が `top_p` に達するトークンからのみサンプリングします。
</ParamField>

<ParamField body="tools" type="array">
  モデルが呼び出せるツールのリスト。現在は `type: "function"` タイプのみサポートし、`web_search`、`file_search`、`computer_use` などの組み込みツールはサポートしていません。
  ...
  <Expandable title="function ツールの形式">
    ...
    <ParamField body="name" type="string" required>
      関数名で、`^[a-zA-Z0-9_-]{1,64}$` に準拠します
    </ParamField>

    <ParamField body="description" type="string">
      関数の説明。モデルがこのツールをいつ呼び出すか判断するのに役立ちます
    </ParamField>

    <ParamField body="parameters" type="object">
      パラメータ定義で、JSON Schema形式に準拠しています
    </ParamField>
  </Expandable>
</ParamField>
...
<ParamField body="tool_choice" type="string | object" default="auto">
  ...
  - `"auto"`：モデルがツール呼び出しを自動判断
  - `"none"`：ツール呼び出し禁止
  - `"required"`：最低1つのツール呼び出しを強制
  - `{ "type": "function", "name": "function_name" }`：特定のツール呼び出しを強制
</ParamField>

<ParamField body="parallel_tool_calls" type="boolean" default="true">
  モデルが複数のツールを並行して呼び出せるかどうか。`tools` が提供されている場合のみ有効です。
</ParamField>
    ...
    <ParamField body="format" type="object">
      <Expandable title="format オブジェクト">
        <ParamField body="type" type="string" required>
          ...
          - `"json_schema"`：指定Schemaに準拠したJSON
        </ParamField>

        <ParamField body="name" type="string">
          Schemaの名前（`json_schema` の場合のみ有効）
        </ParamField>
        ...
        <ParamField body="strict" type="boolean" default="true">
          Schemaを厳密に遵守するかどうか（`json_schema` の場合のみ有効）
        </ParamField>
...
<ResponseField name="id" type="string">
  レスポンスの一意識別子。形式：`resp_` + 24桁のランダム文字列
</ResponseField>
...
<ResponseField name="created_at" type="number">
  レスポンス作成時間。Unixタイムスタンプ（秒、ミリ秒精度含む）
</ResponseField>
...
<ResponseField name="status" type="string">
  ...
  - `"in_progress"`：ストリーミング出力進行中（ストリーミングイベントのみ）
  - `"failed"`：生成失敗
</ResponseField>
...
<ResponseField name="task_id" type="string">
  プラットフォーム拡張フィールド。この呼び出しの課金タスクIDで、照合や消費確認に使用可能です。

  **注意**：
  - **非ストリーミング**: `task_id` はレスポンスJSONのトップレベルにあります
  - **ストリーミング**: `task_id` は `response.created`（最初のイベント）および `response.completed`（最後のイベント）の `response` オブジェクト内にネストされており、SSEイベントの生データから解析が必要です。OpenAI SDKのストリーミングインターフェースはこのフィールドを自動で公開しません
</ResponseField>

<ResponseField name="output" type="array">
  出力内容の配列で、テキストメッセージとツール呼び出しの2種類を含みます。

  <Expandable title="message タイプ">
    <ResponseField name="id" type="string">
      メッセージID。形式：`msg_` + 16桁のランダム文字列
    </ResponseField>
  ...
  <Expandable title="function_call タイプ">
    <ResponseField name="id" type="string">
      ツール呼び出しエントリーID。形式：`fc_` + 16桁のランダム文字列
    </ResponseField>
    ...
    <ResponseField name="arguments" type="string">
      関数パラメータ。JSONシリアライズされた文字列
    </ResponseField>
  </Expandable>
</ResponseField>

<ResponseField name="output_text" type="string">
  ショートカットフィールド。`output[0].content[0].text` と同等（プレーンテキストシナリオ）。ツール呼び出しシナリオでは空文字列になります。
</ResponseField>
...
<ResponseField name="incomplete_details" type="object | null">
  `status` が `"incomplete"` の場合に null ではありません。

  <Expandable title="incomplete_details オブジェクト">
    <ResponseField name="reason" type="string">
      トランケーション理由。現在は `"max_output_tokens"` のみ
    </ResponseField>
  </Expandable>
</ResponseField>
...
response = client.responses.create(
    ...
    input="Pythonで文字列を逆にするにはどうすればいいですか？",
)
...
for item in response.output:
    if item.type == "function_call":
        print(f"ツール: {item.name}, 引数: {item.arguments}")
```
...
response = client.responses.create(
    model="claude-sonnet-4-5-20250929",
    input="情報抽出: アリスは30歳で、ハイキングと料理が好きです。",
    text={
        "format": {
            "type":   "json_schema",
            "name":   "person_info",
...
response = client.responses.create(
    ...
    input="解いてください: x + y = 10 かつ x * y = 21 のとき、x と y は？",
...
`previous_response_id` は現在効果がなく、過去のメッセージを `input` に手動で連結する必要があります：
