...
<ParamField body="model" type="string" required>
  使用するモデル名。例：`claude-sonnet-4-5-20250929`、`gpt-4o`。

  完全なモデル一覧は `GET /api/v1/models` で取得可能。
</ParamField>

<ParamField body="input" type="array" required>
  入力内容のリスト

  入力配列で、それぞれの入力項目は `role` と `content` の2つのフィールドを含みます。多段対話やマルチモーダルコンテンツ（テキスト＋画像）をサポート。

  <Expandable title="詳細フィールド説明">
    <ParamField body="role" type="string" required default="user">
      メッセージの役割

      選択可能な値：`user`（ユーザーメッセージ）、`assistant`（AI応答、多段対話用）、`system`（システムプロンプト）
    </ParamField>

    <ParamField body="content" type="array" required>
      コンテンツ配列

      複数タイプのコンテンツブロックをサポートし、テキストや画像を含めることができます。
      ...
      <Expandable title="コンテンツブロックのタイプ">
        ...
        <ParamField body="text" type="string">
          ...
          `type` が `input_text` の場合に使用し、テキスト内容を入力します
        </ParamField>

        <ParamField body="image_url" type="string">
          画像URL

          `type` が `input_image` の場合に使用
          ...
          * サポート画像フォーマット：jpeg、png、gif、webp
          * 例：`data:image/jpeg;base64,/9j/4AAQSkZJRgABAQEAYABg...`
          * 注意：`data:image/jpeg;base64,`のプレフィックス部分を必ず含めること
        </ParamField>
      </Expandable>
    </ParamField>
  </Expandable>
...
<ParamField body="instructions" type="string">
  システムプロンプト（System Prompt）。モデルの行動規範、役割、またはコンテキスト背景を設定するために使用されます。

  メッセージ配列中の `role: "system"` のメッセージと同等です。
</ParamField>

<ParamField body="stream" type="boolean" default="true">
  ストリーミング出力を有効にするかどうか。

  - `true`（デフォルト）：SSEイベントストリーム方式で逐次トークンを返し、リアルタイム表示に適しています
  - `false`：完全なレスポンスを待って一括返却し、バッチ処理に適しています
</ParamField>

<ParamField body="max_output_tokens" type="integer">
  生成される応答の最大トークン数。制限を超えた場合、レスポンスの `status` は `"incomplete"` となり、`incomplete_details.reason` は `"max_output_tokens"` になります。
</ParamField>

<ParamField body="temperature" type="number">
  サンプリング温度、範囲は `0` ～ `2`。値が高いほど出力はランダムになり、値が低いほど決定的な出力になります。`temperature` と `top_p` を同時に変更することは推奨されません。
</ParamField>

<ParamField body="top_p" type="number">
  nucleusサンプリングの確率、範囲は `0` ～ `1`。モデルは累積確率が `top_p` に達したトークンの中からのみサンプリングします。
</ParamField>

<ParamField body="tools" type="array">
  モデルが呼び出せるツールのリスト。現在は `type: "function"` タイプのみサポートしており、`web_search`、`file_search`、`computer_use` などの組み込みツールはサポートしていません。

  <Expandable title="functionツールの形式">
    <ParamField body="type" type="string" required>
      固定で `"function"`
    </ParamField>

    <ParamField body="name" type="string" required>
      関数名、`^[a-zA-Z0-9_-]{1,64}$` に準拠
    </ParamField>

    <ParamField body="description" type="string">
      関数の説明。モデルがいつツールを呼び出すか判断するためのもの
    </ParamField>

    <ParamField body="parameters" type="object">
      パラメータ定義。JSON Schema形式に準拠
    </ParamField>
  </Expandable>
</ParamField>
...
<ParamField body="parallel_tool_calls" type="boolean" default="true">
  モデルが複数のツールを並行して呼び出せるかどうか。`tools` が提供されている場合のみ有効。
</ParamField>
    ...
    <ParamField body="format" type="object">
      <Expandable title="formatオブジェクト">
        ...
        <ParamField body="name" type="string">
          Schema名（`json_schema` 時のみ有効）
        </ParamField>

        <ParamField body="description" type="string">
          Schema説明（`json_schema` 時のみ有効）
        </ParamField>

        <ParamField body="strict" type="boolean" default="true">
          Schemaを厳格に遵守するかどうか（`json_schema` 時のみ有効）
        </ParamField>

        <ParamField body="schema" type="object">
          JSON Schema定義（`json_schema` 時に必須）
        </ParamField>
      </Expandable>
    </ParamField>
  </Expandable>
