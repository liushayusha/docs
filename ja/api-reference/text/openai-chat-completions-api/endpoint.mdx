...
description: |
    ...
    - マルチターン会話、ツール呼び出し、構造化出力、ストリーミング応答をサポート
    - OpenAI SDK の `baseURL` を直接置き換え可能、他のコード修正不要
...
<ResponseField name="object" type="string">
  オブジェクトタイプ、固定で `"chat.completion"`（非ストリーミング）または `"chat.completion.chunk"`（ストリーミング）
</ResponseField>
...
<ResponseField name="task_id" type="string">
  課金タスクID（プロジェクト拡張フィールド）、今回の呼び出しのポイント消費記録を追跡するため
</ResponseField>
...
<ResponseField name="choices" type="array">
  ...
  <Expandable title="フィールド説明">
    ...
    <ResponseField name="message" type="object">
      **非ストリーミング応答** の完全なメッセージオブジェクト

      * `role` — 固定で `"assistant"`
      * `content` — テキスト内容（ツール呼び出し時は `null`）
      * `tool_calls` — ツール呼び出しリスト（ツール呼び出しがある場合に存在）
    </ResponseField>
...
<ResponseField name="usage" type="object">
  トークン使用統計（非ストリーミングは応答ボディのトップレベル、ストリーミングは最後のフレームに位置）
...
<ResponseExample>
  ...
  {
    "error": {
      "message": "messagesは必須であり、空ではいけません",
...
response = client.chat.completions.create(
    model="claude-sonnet-4-5-20250929",
    messages=[
        {"role": "system",    "content": "あなたは経験豊富なPython開発エキスパートで、コードレビューとパフォーマンス最適化が得意です。"},
        {"role": "user",      "content": "GILとは何ですか？"},
        {"role": "assistant", "content": "GIL（グローバルインタプリタロック）は..."},
        {"role": "user",      "content": "GILを回避して真の並列処理を実現する方法は？"}
    ],
    temperature=0.3
)
```
...
stream = client.chat.completions.create(
    model="claude-sonnet-4-5-20250929",
    messages=[{"role": "user", "content": "量子コンピューティングに関する技術ブログを書いてください"}],
    stream=True
)
...
tools = [
    {
        ...
        "function": {
            ...
            "description": "株価のリアルタイム価格を取得する",
            "parameters": {
                "type": "object",
                "properties": {
                    "ticker": {"type": "string", "description": "株式コード、例えば：AAPL"}
                },
                "required": ["ticker"]
            }
        }
...
response = client.chat.completions.create(
    model="claude-sonnet-4-5-20250929",
    messages=[
        {"role": "user", "content": "以下の製品レビューの感情分析をしてください：'このイヤホンは音質が優れているが、バッテリー持ちは普通だ'"}
    ],
    response_format={
        "type": "json_schema",
        "json_schema": {
            ...
            "schema": {
                ...
                "properties": {
                    ...
                    "score":       {"type": "number", "description": "感情スコア -1.0から1.0まで"},
...
response = client.chat.completions.create(
    model="claude-sonnet-4-5-20250929",
    messages=[{"role": "user", "content": "1234 × 5678 の結果を計算してください"}],
    temperature=0.0,
    seed=42
)
```
