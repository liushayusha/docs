...
<ParamField header="Authorization" type="string" required>
  すべてのインターフェースはBearer Tokenを使用した認証が必要です
...
<ResponseField name="input" type="object">
  タスク入力(オプション、特定の場合にのみ返される)

  タスク送信時の完全なパラメータ情報を含む
</ResponseField>

<ResponseField name="output" type="array">
  タスク出力(タスク完了後に返される)

  生成結果を含む配列、各要素は生成されたリソースを指す`url`フィールドを含む
</ResponseField>
  ...
  ```json 200
{
  ...
  ```json 500
  {
    "error": {
      "code": 500,
      "message": "サーバー内部エラー、しばらくしてからもう一度お試しください"
    }
  }
  ```
</ResponseExample>
