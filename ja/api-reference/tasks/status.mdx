...
<ParamField body="task_id" type="string">
  ...
  out_task_idとどちらか一方を選択し、どちらか一方を必ず提供してください
</ParamField>

<ParamField body="out_task_id" type="string">
  発注側のタスクID

  task_idとどちらか一方を選択し、どちらか一方を必ず提供してください
</ParamField>
...
<ResponseField name="input" type="object">
  タスク入力（オプションで、一部の場合のみ返されます）

  タスクを提出したときの完全なパラメータ情報を含みます
</ResponseField>

<ResponseField name="output" type="array">
  タスク出力（タスク完了後に返されます）

  生成結果の配列で、各要素は生成されたリソースへの `url` フィールドを含みます
</ResponseField>
