---
title: 'タスクのステータスを取得する'
description: |
    - 非同期タスクの実行ステータスと結果をクエリする
    - リアルタイムステータス更新と進捗追跡
    - タスク完了時に生成結果を取得する
...
<ResponseField name="input" type="object">
  タスク入力（オプション、一部の場合のみ返されます）

  タスク提出時の完全なパラメータ情報を含みます
</ResponseField>

<ResponseField name="output" type="array">
  タスク出力（タスク完了後に返されます）

  生成結果の配列を含み、各要素は生成されたリソースを指す `url` フィールドを持っています
</ResponseField>
