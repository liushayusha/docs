...
<ParamField body="model" type="string" required>
  ...
  * `"nano_banana"` - 기본
  * `"nano_banana_base"` - 기본 버전
  * `"nano_banana_plus"` - 강화 버전
  * `"nano_banana_max"` - 플래그십 버전
</ParamField>
...
<ParamField body="params" type="object" required>
  ...
  <Expandable title="params 객체 속성">
    ...
    <ParamField body="image_url" type="string[]">
      이미지 생성에 참고하는 이미지의 URL 목록

      * 최대 9개의 이미지
      * 다음 두 가지 형식을 지원:
        1. 공개적으로 접근 가능한 URL
        2. Base64 인코딩 형식
           * 전체 Data URI 형식을 사용해야 합니다: `data:image/{포맷};base64,{base64데이터}`
           * 지원되는 이미지 포맷: jpeg, png, webp
           * 예시: `data:image/jpeg;base64,/9j/4AAQSkZJRgABAQEAYABg...`
           * ⚠️ 주의: `data:image/jpeg;base64,` 접두사가 포함되어야 하며, base64 이미지 크기는 10MB를 초과할 수 없습니다
    </ParamField>
