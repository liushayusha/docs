...
description: |
    ...
    - 최대 16장의 참조 이미지 허용, URL과 base64 혼합 가능
    - 제출된 프롬프트는 플랫폼의 민감어 및 보안 심사를 거치며, 위반 콘텐츠는 즉시 거부
...
<ParamField body="params" type="object" required>
  ...
  <Expandable title="params 객체 속성">
    <ParamField body="prompt" type="string" required>
      ...
      * 중국어와 영어 지원, 자세한 설명 권장
      * 제출 전에 플랫폼의 민감어 / 보안 검사를 거치며, 위반 내용이 포함되면 오류가 바로 반환됨
    </ParamField>

    <ParamField body="image_url" type="string[]">
      참조 이미지 배열, 입력 시 이미지 생성 모드로 전환

      * 최대 16장의 참조 이미지
      * 다음 두 가지 형식 지원:
        1. 공개 접근 가능한 URL
        2. Base64 인코딩 형식
           * 완전한 Data URI 형식 필수: `data:image/{형식};base64,{base64데이터}`
    ...
    <ParamField body="aspect_ratio" type="string" default="1:1">
      ...
      * `4:3` / `3:4` - 표준 가로 / 세로
      * `3:2` / `2:3` - 클래식 가로 / 세로
      * `5:4` / `4:5` - 근접 정방형 가로 / 세로
      * `2:1` / `1:2` - 와이드 가로 / 긴 세로
      * `21:9` / `9:21` - 초와이드 가로 / 초긴 세로

      <Warning>
        비율 표기만 지원하며 픽셀 크기(예: `1024x1024`) 입력 시 오류 발생
      </Warning>
    </ParamField>
  </Expandable>
</ParamField>
