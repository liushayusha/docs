...
<ParamField body="params" type="object" required>
  ...
  <Expandable title="params 객체 속성">
    <ParamField body="prompt" type="string" required>
      ...
      * 한중영 지원, 상세 설명 권장
      * 제출 전 플랫폼 민감어 / 보안 검토가 진행되며, 위반 내용 적발 시 즉시 오류 반환
    </ParamField>

    <ParamField body="image_url" type="string[]">
      참조 이미지 배열, 전달 시 이미지 기반 생성 모드로 처리

      * 최대 9장 참조 이미지 지원
      * 다음 두 가지 형식 지원:
        1. 공개 접근 가능한 URL
        2. Base64 인코딩 형식
           * 반드시 완전한 Data URI 형식 사용: `data:image/{포맷};base64,{base64데이터}`
    ...
    <ParamField body="aspect_ratio" type="string" default="1:1">
      ...
      * `4:3` / `3:4` - 표준 가로 / 세로
      * `3:2` / `2:3` - 클래식 가로 / 세로
      * `5:4` / `4:5` - 근사 정방형 가로 / 세로
      * `2:1` / `1:2` - 넓은 가로 / 긴 세로
      * `21:9` / `9:21` - 초와이드 가로 / 초긴 세로

      <Warning>
        비율 문자열 또는 `auto` 만 지원하며, 픽셀 크기(예: `1024x1024`) 전달 시 직접 오류 발생
      </Warning>
    </ParamField>
      ...
      * `4K` - 3840 기준, **6가지 비율만 지원** (`16:9` / `9:16` / `2:1` / `1:2` / `21:9` / `9:21`)
      ...
      <Warning>
        ...
        * `5:4` / `4:5` × `4K` ❌ (픽셀 초과 한도)
        ...
        위 비율에서 고화질 원할 경우 `resolution=2K` 사용 권장
      </Warning>
    </ParamField>
  </Expandable>
</ParamField>
