...
<ParamField body="model" type="string" required>
  모델 이름, 다음 채널 버전을 지원합니다(채널은 서비스 안정성에만 영향을 주며 출력 품질에는 영향을 주지 않습니다):

  * `"nano_banana_v2"` - 기본
  * `"nano_banana_v2_base"` - 기본 버전
  * `"nano_banana_v2_plus"` - 향상 버전
  * `"nano_banana_v2_max"` - 플래그십 버전
</ParamField>
...
<ParamField body="params" type="object" required>
  ...
  <Expandable title="params 객체 속성">
    ...
    <ParamField body="image_url" type="string[]">
      참조 이미지 URL 목록, 이미지에서 이미지로 변환에 사용

      * 최대 8개의 이미지
      * 다음 두 가지 형식을 지원합니다:
        1. 공개 액세스 가능 URL
        2. Base64 인코딩 형식
           * 전체 데이터 URI 형식을 사용해야 합니다: `data:image/{포맷};base64,{base64데이터}`
           * 지원 이미지 형식: jpeg, png, webp
           * 예시: `data:image/jpeg;base64,/9j/4AAQSkZJRgABAQEAYABg...`
           * ⚠️ 참고: `data:image/jpeg;base64,` 접두사가 반드시 포함되어야 하며, base64 이미지 크기는 10MB를 초과할 수 없습니다
    </ParamField>
