...
<ParamField body="model" type="string" default="gpt-image-2-official" required>
  ...
  고정 입력 값 `gpt-image-2-official`(OpenAI 공식 gpt-image-2 모델)
</ParamField>
...
<ParamField body="params" type="object" required>
  ...
  <Expandable title="params 객체 속성">
    <ParamField body="prompt" type="string" required>
      ...
      * 한글, 영어 모두 지원, 자세한 설명 권장
      * 제출 전 플랫폼 내 민감어 / 보안 심사 진행, 위배 내용 발견 시 직접 오류 반환
    </ParamField>

    <ParamField body="size" type="string" default="1:1">
      화면 비율

      외부에 사용하는 비율 값, 시스템 내부에서는 `resolution`에 따라 자동으로 구체 픽셀에 매핑

      13가지 비율 지원:

      * `1:1` - 정사각형 구도(기본, 소셜 프로필 사진 / 로고)
      * `3:2` - 가로 구도(DSLR 카메라 일반 비율)
      * `2:3` - 세로 구도(포스터 세로형)
      * `4:3` - 가로 구도(클래식 모니터 / PPT)
      * `3:4` - 세로 구도
      * `5:4` - 가로 구도
      * `4:5` - 세로 구도(인스타그램 세로 게시물)
      * `16:9` - 가로 구도(와이드스크린 영상 커버)
      * `9:16` - 세로 구도(휴대폰 전체 화면 / 숏폼 영상 커버)
      * `2:1` - 가로 구도(웹 배너)
      * `1:2` - 세로 구도
      * `21:9` - 가로 구도(영화 초광폭 화면)
      * `9:21` - 세로 구도

      <Warning>
        비율 형식만 지원하며, 픽셀 사이즈(ex: `1024x1024`) 전송 시 직접 오류 발생
      </Warning>
    </ParamField>
    ...
    <ParamField body="resolution" type="string" default="1K">
      ...
      * `1K` - 1024 기준, 비용 절감 및 일상 사용 적합(기본값)
      ...
      * `4K` - 3840 기준, **6개 비율만 지원**(`16:9` / `9:16` / `2:1` / `1:2` / `21:9` / `9:21`)
      ...
      <Warning>
        ...
        * `5:4` / `4:5` × `4K` ❌ (초과 픽셀 제한)
        ...
        고해상도로 위 비율 사용 시, `resolution=2K` 사용 권장
      </Warning>
    </ParamField>
    ...
    <ParamField body="quality" type="string" default="auto">
      ...
      * `auto` - 자동(기본값)
      * `low` - 빠르고 비용 절감, 윤곽선 표현에 적합
      * `medium` - 균형
      * `high` - 최고 품질(4K + high 시 소요 시간 >120초)
    </ParamField>

    <ParamField body="image_urls" type="array">
      참고 이미지 배열, 입력 시 이미지 참고 생성 모드 활성화

      * 최대 **9장** 참고 이미지 허용
      * 다음 두 가지 형식 지원:
        1. 공개 접근 가능한 URL
        2. Base64 인코딩 형식
           * 반드시 전체 Data URI 형식 사용: `data:image/{포맷};base64,{base64데이터}`
           * 지원 이미지 포맷: jpeg, png, webp
           * 예시: `data:image/jpeg;base64,/9j/4AAQSkZJRgABAQEAYABg...`
           * ⚠️ 주의: 반드시 `data:image/jpeg;base64,` 접두사 포함
    </ParamField>
  </Expandable>
</ParamField>
...
제출 성공 후 `task_id`가 반환되며, `GET /v1/tasks/{task_id}`를 통해 작업 상태를 폴링할 수 있습니다. 자세한 내용은 [작업 조회 API](/en/api-reference/tasks/get-task)를 참고하세요.

### 폴링 권장사항

* **최초 조회 지연 시간**：제출 후 10~20초 대기 후 조회 시작
* **조회 간격**：3~5초 간격 권장
* **타임아웃 참고**：`high + 2K/4K` 조합 처리 시간은 최대 130초일 수 있으며, 클라이언트 타임아웃은 180초 이상 권장
...
<ResponseExample>
  ...
  ```json 400
  {
    "error": {
      "code": 400,
      "message": "크기 1:1에 대해 해상도 4K는 지원되지 않으며, 허용치는 1K / 2K입니다",
  ...
  ```json 429
  {
    "error": {
      "code": 429,
      "message": "요청이 너무 빈번합니다. 잠시 후 다시 시도하세요",
