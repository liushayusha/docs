...
<ParamField body="model" type="string" required>
  ...
  * `"nano_banana_pro"` - 기본값
  * `"nano_banana_pro_base"` - 기본 버전
  * `"nano_banana_pro_plus"` - 향상된 버전
  * `"nano_banana_pro_advanced"` - 고급 버전
  * `"nano_banana_pro_max"` - 플래그십 버전
</ParamField>
...
<ParamField body="params" type="object" required>
  ...
  <Expandable title="params 객체 속성">
    ...
    <ParamField body="image_url" type="string[]">
      참조 이미지 URL 리스트, 이미지 기반 생성에 사용

      * 최대 8개의 이미지
      * 다음 두 가지 형식 지원:
        1. 공개 접근 가능한 URL
        2. Base64 인코딩 형식
           * 전체 Data URI 형식 사용 필수: `data:image/{포맷};base64,{base64데이터}`
           * 지원 이미지 형식: jpeg, png, webp
           * 예시: `data:image/jpeg;base64,/9j/4AAQSkZJRgABAQEAYABg...`
           * ⚠️ 주의: 반드시 `data:image/jpeg;base64,` 접두사를 포함해야 하며, base64 이미지 크기는 10MB를 초과할 수 없습니다
    </ParamField>
