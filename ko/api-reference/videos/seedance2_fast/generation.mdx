...
- `seedance2_fast` — 표준 빠른 버전
- `seedance2_fast_face` — 실제 인물 업로드 지원, 빠른 버전과 동일한 기능을 제공합니다
</Note>
...
<ParamField body="model" type="string" required>
  ...
  다음 모델 변형을 지원합니다(매개변수 완전히 일치하며 필요에 따라 선택):
  * `seedance2_fast` — 표준 빠른 버전
  * `seedance2_fast_face` — 실제 인물 업로드 지원, 빠른 버전과 기능 일치
</ParamField>
...
<ParamField body="params" type="object" required>
  ...
  <Expandable title="params 객체 속성">
    ...
    <ParamField body="type" type="string" required>
      ...
      * `all_reference` — 스마트 참조 모드: `image_url`은 캐릭터/스타일 참조 이미지(최대 9장), `video_url`은 참조 비디오(최대 3개)로 사용
      * `first_last_frame` — 시작/끝 프레임 모드: `image_url`은 시작 프레임(단 1장), `end_image_url`은 끝 프레임, `video_url` 지원 안함

      필수, 선택 값: `all_reference`, `first_last_frame`
    </ParamField>

    <ParamField body="image_url" type="string[]">
      참조 이미지 URL

      * `all_reference` 모드: 최대 9장, 각 이미지 최대 50MB, 최소 가로세로 300px
      * `first_last_frame` 모드: 단 1장(시작 프레임), 최대 10MB, 최소 가로세로 300px
    </ParamField>

    <ParamField body="video_url" type="string[]">
      참조 비디오 URL

      `all_reference` 모드만 지원. 최대 3개; `mp4`, `mov` 지원; 개별 비디오 길이 2~15초, 참조 비디오 전체 길이 합은 15초 초과 불가; 단일 비디오 최대 50MB; 프레임률 24~60 FPS; 해상도 `480p`, `720p` 지원; 가로세로 비율 0.4~2.5; 가로세로 범위 300~6000px; 총 픽셀 수 409600~927408 사이여야 함

      <Warning>`all_reference` 모드에서만 유효, `type`이 `first_last_frame`일 경우 이 매개변수를 전달하지 마세요</Warning>
    </ParamField>
