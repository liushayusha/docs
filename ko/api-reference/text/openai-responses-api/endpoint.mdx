...
<ParamField body="model" type="string" required>
  사용할 모델 이름입니다. 예: `gpt-4o`, `gpt-5.4` 등.

  전체 모델 목록은 `GET /api/v1/models`로 확인할 수 있습니다.
</ParamField>

<ParamField body="input" type="array" required>
  입력 내용 목록

  입력 배열로, 각 항목은 `role`과 `content` 두 필드를 포함합니다. 다중 대화 및 다중 모달 콘텐츠(텍스트+이미지)를 지원합니다.

  <Expandable title="상세 필드 설명">
    <ParamField body="role" type="string" required default="user">
      메시지 역할

      선택 가능한 값: `user`(사용자 메시지), `assistant`(AI 답변, 다중 대화용), `system`(시스템 프롬프트)
    </ParamField>
    ...
    <ParamField body="content" type="array" required>
      ...
      <Expandable title="콘텐츠 블록 유형">
        ...
        <ParamField body="image_url" type="string">
          ...
          * 공개 액세스 가능한 이미지 URL(http\:// 또는 https\://)
...
<ParamField body="instructions" type="string">
  시스템 프롬프트(System Prompt)입니다. 모델의 동작 원칙, 역할 아이덴티티 또는 컨텍스트 배경을 설정하는 데 사용됩니다.

  메시지 배열 내에서 `role: "system"`에 해당하는 메시지와 동일합니다.
</ParamField>

<ParamField body="stream" type="boolean" default="true">
  스트리밍 출력을 활성화할지 여부입니다.

  - `true`(기본값): SSE 이벤트 스트림 방식으로 토큰 단위로 반환되어 실시간 표시용으로 적합합니다.
  - `false`: 전체 응답을 기다렸다가 한 번에 반환하며, 배치 처리에 적합합니다.
</ParamField>

<ParamField body="max_output_tokens" type="integer">
  생성되는 응답의 최대 토큰 수입니다. 제한을 초과하면 응답 `status`가 `"incomplete"`가 되고, `incomplete_details.reason`은 `"max_output_tokens"`가 됩니다.
</ParamField>

<ParamField body="temperature" type="number">
  샘플링 온도, 범위 `0` ~ `2`. 값이 높을수록 출력이 더 무작위적이며, 값이 낮을수록 결정적입니다. `temperature`와 `top_p`를 동시에 수정하는 것은 권장하지 않습니다.
</ParamField>

<ParamField body="top_p" type="number">
  핵심 샘플링 확률, 범위 `0` ~ `1`. 모델은 누적 확률이 `top_p`에 도달하는 토큰 범위 내에서만 샘플링합니다.
</ParamField>

<ParamField body="tools" type="array">
  모델이 호출 가능한 도구 목록입니다. 현재 `type: "function"` 유형만 지원하며, `web_search`, `file_search`, `computer_use` 등 내장 도구는 지원하지 않습니다.
  ...
  <Expandable title="function 도구 형식">
    ...
    <ParamField body="name" type="string" required>
      함수 이름이며, `^[a-zA-Z0-9_-]{1,64}$` 형식을 준수해야 합니다.
    </ParamField>
    ...
    <ParamField body="parameters" type="object">
      파라미터 정의이며, JSON Schema 형식을 준수합니다.
    </ParamField>
  </Expandable>
</ParamField>
...
<ParamField body="tool_choice" type="string | object" default="auto">
  ...
  - `"auto"`: 모델이 자동으로 도구 호출 여부를 결정합니다.
  - `"none"`: 도구 호출 금지
  - `"required"`: 최소 한 개 도구 호출 강제
  - `{ "type": "function", "name": "function_name" }`: 지정한 도구 호출 강제
</ParamField>
    ...
    <ParamField body="format" type="object">
      <Expandable title="format 객체">
        ...
        <ParamField body="schema" type="object">
          JSON Schema 정의(`json_schema`일 때 필수)
        </ParamField>
      </Expandable>
    </ParamField>
  </Expandable>
...
<ParamField body="reasoning" type="object">
  추론 모드 구성(예: `gpt-5.2` 이상 지원 모델에 적용).
