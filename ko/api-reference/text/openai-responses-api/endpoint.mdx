...
<ParamField body="model" type="string" required>
  사용하려는 모델 이름입니다. 예: `claude-sonnet-4-5-20250929`, `gpt-4o`.

  전체 모델 목록은 `GET /api/v1/models`를 통해 확인할 수 있습니다.
</ParamField>

<ParamField body="input" type="array" required>
  입력 내용 목록

  입력 배열이며, 각 입력 항목은 `role`과 `content` 두 필드를 포함합니다. 다중 회차 대화 및 다중 모달(텍스트+이미지) 내용을 지원합니다.

  <Expandable title="상세 필드 설명">
    <ParamField body="role" type="string" required default="user">
      메시지 역할

      선택 값: `user`(사용자 메시지), `assistant`(AI 응답, 다중 회차 대화용), `system`(시스템 프롬프트)
    </ParamField>

    <ParamField body="content" type="array" required>
      내용 배열

      다양한 유형의 콘텐츠 블록을 포함할 수 있으며, 텍스트와 이미지를 포함할 수 있습니다.

      <Expandable title="내용 블록 타입">
        ...
        <ParamField body="text" type="string">
          ...
          `type`이 `input_text`일 때 사용하며, 텍스트 내용을 작성합니다.
        </ParamField>

        <ParamField body="image_url" type="string">
          이미지 URL

          `type`이 `input_image`일 때 사용합니다.
...
<ParamField body="instructions" type="string">
  시스템 프롬프트(System Prompt)입니다. 모델의 동작 원칙, 역할 또는 문맥 배경을 설정하는 데 사용됩니다.

  메시지 배열 중 `role: "system"` 메시지와 동일한 역할을 합니다.
</ParamField>

<ParamField body="stream" type="boolean" default="true">
  스트리밍 출력 활성화 여부.

  - `true`(기본값): SSE 이벤트 스트림 방식으로 토큰 별로 반환되어 실시간 표시용으로 적합
  - `false`: 전체 응답을 대기 후 한 번에 반환, 배치 처리에 적합
</ParamField>

<ParamField body="max_output_tokens" type="integer">
  생성할 응답의 최대 토큰 수입니다. 제한을 초과하면 응답 `status`가 `"incomplete"`, `incomplete_details.reason`이 `"max_output_tokens"`로 반환됩니다.
</ParamField>

<ParamField body="temperature" type="number">
  샘플링 온도, 범위 `0` ~ `2`입니다. 값이 높을수록 출력이 더 무작위적이고, 낮을수록 더 결정적입니다. `temperature`와 `top_p`를 동시에 변경하는 것은 권장하지 않습니다.
</ParamField>

<ParamField body="top_p" type="number">
  핵 샘플링 확률, 범위 `0` ~ `1`입니다. 모델은 누적 확률이 `top_p`에 도달한 토큰 집합에서만 샘플링합니다.
</ParamField>

<ParamField body="tools" type="array">
  모델이 호출할 수 있는 도구 목록입니다. 현재는 `type: "function"` 유형만 지원하며, `web_search`, `file_search`, `computer_use` 등의 내장 도구는 지원하지 않습니다.
  ...
  <Expandable title="function 도구 형식">
    ...
    <ParamField body="name" type="string" required>
      함수 이름으로, `^[a-zA-Z0-9_-]{1,64}$` 패턴을 따라야 합니다.
    </ParamField>
    ...
    <ParamField body="parameters" type="object">
      파라미터 정의로, JSON Schema 형식을 준수합니다.
    </ParamField>
  </Expandable>
</ParamField>
...
<ParamField body="tool_choice" type="string | object" default="auto">
  ...
  - `"auto"`: 모델이 도구 호출 여부를 자동 결정함
  - `"none"`: 도구 호출 금지
  - `"required"`: 최소 한 개 도구를 반드시 호출
  - `{ "type": "function", "name": "function_name" }`: 지정한 도구를 강제로 호출
</ParamField>

<ParamField body="parallel_tool_calls" type="boolean" default="true">
  모델이 여러 도구를 병렬로 호출할 수 있는지 여부. `tools`가 제공된 경우에만 유효합니다.
</ParamField>
    ...
    <ParamField body="format" type="object">
      <Expandable title="format 객체">
        <ParamField body="type" type="string" required>
          ...
          - `"text"`: 일반 텍스트(기본값)
          - `"json_object"`: JSON 형식
          - `"json_schema"`: 지정된 스키마에 부합하는 JSON
        </ParamField>
        ...
        <ParamField body="strict" type="boolean" default="true">
          스키마 준수 엄격 여부 (`json_schema`일 때만 유효)
        </ParamField>
...
<ParamField body="reasoning" type="object">
  추론 모드 구성 (추론을 지원하는 모델, 예: `gpt-5.2` 이상에 적용).
