---
title: 'AIReiter — AI 文本、图像与视频生成 API 平台'
description: '统一的 AI 文本、图像和视频生成 API 平台。支持多种主流模型，包括 Claude、GPT、Gemini、VEO3、Seedance2 等。兼容 OpenAI SDK，异步任务处理，透明定价，简单易用。'
---

import { apiKeyUrl } from '/snippets/_variables.mdx';

AIReiter 是一个统一的 AI 内容生成 API 平台，提供文本对话、图像生成和视频生成服务。通过简单的 REST API 调用，即可使用 Claude、GPT、Gemini、VEO3、Seedance2 等多种主流 AI 模型。文本接口完全兼容 OpenAI SDK，可直接替换 `baseURL` 使用。
...
<CardGroup cols={2}>
  ...
  <Card title="视频生成" icon="video" href="/zh/api-reference/videos/veo3-1/generation">
    使用 VEO3、Seedance2 等模型生成视频内容
  </Card>
...
使用时只需将模型名称替换为对应的通道版本即可，例如将 `"seedance2"` 替换为 `"seedance2_fast"`，其他参数保持不变。
...
<CardGroup cols={2}>
  ...
  <Card title="查看视频生成文档" icon="video" href="/zh/api-reference/videos/veo3-1/generation">
    了解如何生成视频内容
  </Card>
