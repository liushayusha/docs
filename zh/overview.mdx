...
description: '统一的 AI 文本、图像和视频生成 API 平台。支持多种主流模型，包括 Claude、GPT、Gemini、Sora2、VEO3 等。兼容 OpenAI SDK，异步任务处理，透明定价，简单易用。'
---

import { apiKeyUrl } from '/snippets/_variables.mdx';

AIReiter 是一个统一的 AI 内容生成 API 平台，提供文本对话、图像生成和视频生成服务。通过简单的 REST API 调用，即可使用 Claude、GPT、Gemini、Sora2、VEO3 等多种主流 AI 模型。文本接口完全兼容 OpenAI SDK，可直接替换 `baseURL` 使用。
