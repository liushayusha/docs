...
所有图像和视频生成都采用异步处理模式。提交任务时需要传入 `out_task_id`（发起方任务ID），系统会返回 `task_id` 和 `out_task_id`：
...
使用返回的 `task_id` 或您提交时的 `out_task_id` 查询任务进度和结果（二选一）：
...
- 立即返回系统生成的 `task_id` 和您提交的 `out_task_id`
- 可使用 `task_id` 或 `out_task_id` 查询任务状态（二选一）
- 支持 webhook 回调通知（可选）
...
<AccordionGroup>
  ...
  <Accordion title="如何查询任务状态？">
    使用任务状态查询 API，可以传入提交任务时返回的 `task_id`，或者您自己设置的 `out_task_id`（二选一）：
  ...
  <Accordion title="什么是 out_task_id？">
    ...
    - **查询灵活**：可以直接使用 `out_task_id` 查询任务状态，无需保存系统返回的 `task_id`
    - **结果关联**：在查询结果中会同时返回 `task_id` 和 `out_task_id`，方便您关联业务数据
  </Accordion>
