...
description: '• 异步处理模式，返回任务 ID 以供后续查询<br/>• 高质量图像生成，专业级图像创作<br/>• 生成的图片链接有效期为24小时，请及时保存'
...
<ParamField body="n" type="integer">
  ...
  * 实际可生成的图像数量受此参数和输入参考图像数量双重影响。参考图像 + 生成图像 总和必须 ≤ 15
  * **必须输入纯数字（例如 `1`），不要使用引号，否则会导致错误**
...
<ParamField body="optimize_prompt_options.mode" type="string" default="standard">
  ...
  * `standard`：标准模式，生成更高质量内容但处理时间更长
  * `fast`：快速模式，生成内容时间更短但质量一般
...
<ParamField body="sequential_image_generation" type="string" default="disabled">
  ...
  * `disabled`：禁用顺序模式，即使有多张参考图也只生成1张图（默认）
  * `auto`：启用顺序模式，可生成多张图

  **使用说明：**

  * ✅ 必须提供 `image_urls` - 至少一张参考图
  ...
  * 纯文本生成图像（无 `image_urls`）无法生成多张图 - 这是Doubao API的限制
</ParamField>
...
<ParamField body="sequential_image_generation_options" type="object">
  ...
  * `max_images`（整数）：指定生成图像数量，范围：1-15
