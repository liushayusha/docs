...
<ParamField header="Authorization" type="string" required >
  所有API都需要使用Bearer Token进行身份验证
</ParamField>

获取API Key:

访问 [API Key Management Page](https://apimart.ai/console/token) 获取您的API Key
...
<ParamField body="resolution" type="string">
  ...
  支持的值:
    ...
    **⚠️ 注意:** 使用base64格式生成4K图片处理时间更长
</ParamField>

<ParamField body="image_urls" type="array">
  用于图像到图像或图像编辑的参考图片URL列表

  💡 **快速填写（试用区）:**
  1. 点击“+ 添加项目”添加图片URL
  2. 输入完整的图片URL地址或base64数据
