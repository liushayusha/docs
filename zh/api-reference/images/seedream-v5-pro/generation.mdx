---
title: 'Seedream V5 Pro 图片生成'
description: |
    - 异步处理模式，返回任务ID用于后续查询
    - 面向高精度生图场景，更精准的位置与元素控制
    - 支持文生图、单图/多参考图生图与自定义输出尺寸
  ...
  <Expandable title="params 对象属性">
    ...
    <ParamField body="aspect_ratio" type="string">
      ...
      * `21:9` - 超宽屏
      ...
      ⚠️ 与 `size` 参数互斥：传入 `size` 自定义尺寸时，`aspect_ratio` 不生效
    </ParamField>
    ...
    <ParamField body="size" type="string">
      自定义输出尺寸，格式 `宽x高`（像素值），如 `2048x1024`

      * 宽高均须为 16 的倍数
      * 总像素（宽×高）范围：[921600, 4194304]（即 1280x720 ~ 2048x2048）
      * 宽高比范围：[1/16, 16]
      * 与 `aspect_ratio` / `resolution` 档位方式互斥，传入后按该像素值精确出图
    </ParamField>
  </Expandable>
</ParamField>
...
<RequestExample>
  ...
  curl --request POST \
    ...
    --data '{
      ...
      "params": {
        "prompt": "一只橘猫坐在窗台上，窗外是东京夜景，赛博朋克风格",
        "aspect_ratio": "16:9",
        "resolution": "2K"
      },
