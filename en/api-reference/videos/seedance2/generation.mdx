...
<ParamField body="params" type="object" required>
  ...
  <Expandable title="params Object Properties">
    ...
    <ParamField body="type" type="string" required>
      ...
      * `first_last_frame` — First and last frame mode: `image_url` is the first frame (only 1 image), `end_image_url` is the last frame, does not support `video_url`

      Required, possible values: `all_reference`, `first_last_frame`
    </ParamField>

    <ParamField body="image_url" type="string[]">
      Reference image URLs

      * In `all_reference` mode: up to 9 images, max 50MB each, minimum dimension 300px
      * In `first_last_frame` mode: only 1 image (first frame), max 10MB, minimum dimension 300px
    </ParamField>

    <ParamField body="video_url" type="string[]">
      Reference video URLs

      Supported only in `all_reference` mode. Up to 3 videos; supports `mp4`, `mov`; single video duration between 2-15 seconds; total duration of all reference videos no more than 15 seconds; max single video size 50MB; frame rate 24-60 FPS; resolutions supported: `480p`, `720p`; aspect ratio 0.4-2.5; width and height range 300-6000px; total pixels between 409600 and 927408

      <Warning>Valid only in `all_reference` mode, please do not provide this parameter when `type` is `first_last_frame`</Warning>
    </ParamField>
    ...
    <ParamField body="end_image_url" type="string">
      ...
      <Note>Valid only in `first_last_frame` mode</Note>
    </ParamField>
