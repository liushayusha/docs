...
<ParamField body="prompt" type="string" required>
  ...
  **💡 @Role Mention Feature:**
  ...
  You can mention created characters in the prompt using the `@username` format to have the character appear in the generated video.

  * The character's `username` can be obtained via the [Query Character Endpoint](/en/api-reference/videos/sora2/query-character)
  * Multiple characters can be mentioned simultaneously

  Example: `"A kitten and a puppy driving a car together @duksvfkf.cruisingki @zdqwahgj.baronbarki"`
</ParamField>
...
<ParamField body="watermark" type="boolean" default="false">
  Whether to add a watermark in the generated video

  * `false`: no watermark added
  * `true`: adds the official Sora watermark in the video
...
<ParamField body="thumbnail" type="boolean">
  ...
  * `true`: generates a video thumbnail for preview and display

  Can be omitted if no thumbnail generation is needed
</ParamField>

<ParamField body="private" type="boolean" default="false">
  Whether to enable privacy mode

  * `true`: the video will not be published and remix (re-edit) operations will be disabled
  * `false`: the video can be published and remixed
...
<ParamField body="storyboard" type="boolean">
  Whether to use storyboard for more detailed control over video generation details
...
<ParamField body="character_url" type="string">
  Character URL from reference video
  ...
  **⚠️ Note:** Using this parameter will reduce the generated video duration by 1 second
</ParamField>

<ParamField body="character_timestamps" type="string">
  Character appearance timestamps

  Specify the time range in the reference video when the character appears; only supports 2-second intervals

  Format: `"start_second,end_second"`

  Example: `"1,3"` indicates the character appears from second 1 to second 3 in the reference video
...
<ResponseField name="data" type="array">
  ...
  <Expandable title="Array Elements">
    ...
    <ResponseField name="task_id" type="string">
      Unique identifier for the task, used to query the task status and results
    </ResponseField>
  </Expandable>
</ResponseField>
...
<RequestExample>
  ...
  curl --request POST \
    ...
    --data '{
      "model": "sora-2",
      "prompt": "A waterfall cascading forming a rainbow",
      "duration": 10,
      "aspect_ratio": "16:9",
      "private": false,
