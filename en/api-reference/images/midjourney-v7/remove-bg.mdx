...
description: |
    ...
    - Remove the image background with one click, generating a clean subject cutout
    - No prompt required, just upload one image
...
<ParamField body="out_task_id" type="string" required>
  Custom task ID from the caller, used for idempotent submission and result retrieval.
