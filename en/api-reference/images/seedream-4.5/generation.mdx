...
description: '• Asynchronous processing mode, returns task ID for subsequent query<br/>• Supports multiple generation modes including text-to-image, image-to-image, image editing, etc.<br/>• Generated image links are valid for 24 hours, please save them promptly'
...
<ParamField body="n" type="integer">
  ...
  * The actual number of images generated is influenced by this parameter and the number of input reference images. Number of reference images + final number of generated images ≤ 15
  * **Must enter a pure number (e.g., `1`), do not add quotes, otherwise an error will occur**

  Fees will be deducted according to the generation quantity
</ParamField>
...
<ParamField body="image_urls" type="array">
  ...
  * ⚠️ Note: Must include the `data:image/jpeg;base64,` prefix part
...
<ParamField body="optimize_prompt_options.mode" type="string" default="standard">
  ...
  * `standard`: Standard mode, higher quality generation, longer time consumption
  * `fast`: Fast mode, shorter generation time, average quality

  Default: `standard`
</ParamField>

<ParamField body="sequential_image_generation" type="string" default="disabled">
  Sequential image generation mode (exclusive Doubao feature)

  Controls whether to generate multiple images:

  * `disabled`: Disable sequential image mode, only generate 1 image even if there are multiple reference images (default)
  * `auto`: Enable sequential image mode, can generate multiple images
