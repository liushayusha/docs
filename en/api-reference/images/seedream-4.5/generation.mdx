...
<ParamField body="n" type="integer">
  ...
  * The actual number of images that can be generated depends not only on this parameter but also on the number of input reference images. Number of reference images + final generated images ≤ 15
  * **Must input a pure number (e.g., `1`), do not add quotes, or an error will occur**

  Charges will be pre-deducted according to the number of images generated
</ParamField>
...
<ParamField body="optimize_prompt_options.mode" type="string" default="standard">
  ...
  * `standard`: Standard mode, higher quality generated content, longer time
  * `fast`: Fast mode, faster generation time, moderate quality
...
<ParamField body="sequential_image_generation" type="string" default="disabled">
  ...
  * `disabled`: Disable sequential mode, even with multiple reference images only 1 image is generated (default)
  ...
  * ⚠️ Automatically sets to `auto` when `n > 1`

  **Limitations:**

  * Pure text-to-image (no `image_urls`) cannot generate multiple images due to Doubao API limitations
</ParamField>
...
<RequestExample>
  ...
  curl --request POST \
    ...
    --data '{
      ...
      "prompt": "Cute panda playing in bamboo forest",
      "size": "1:1",
      "resolution": "2K",
      "n": 1,
      "image_urls": [
