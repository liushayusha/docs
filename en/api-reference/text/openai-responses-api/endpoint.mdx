...
<ParamField body="model" type="string" required>
  ...
  The full list of models can be retrieved via `GET /api/v1/models`.
</ParamField>

<ParamField body="input" type="array" required>
  List of input contents

  An array of inputs, each containing the fields `role` and `content`. Supports multi-turn conversations and multimodal content (text + images).

  <Expandable title="Detailed Field Description">
    <ParamField body="role" type="string" required default="user">
      Message role

      Options: `user` (user message), `assistant` (AI reply, for multi-turn conversation), `system` (system prompt)
    </ParamField>

    <ParamField body="content" type="array" required>
      Content array

      Supports multiple types of content blocks, which can include text and images.
      ...
      <Expandable title="Content Block Types">
        ...
        <ParamField body="image_url" type="string">
          ...
          * Note: The prefix `data:image/jpeg;base64,` must be included
        </ParamField>
      </Expandable>
    </ParamField>
  </Expandable>
...
<ParamField body="stream" type="boolean" default="true">
  ...
  - `true` (default): Returns tokens incrementally via SSE event stream, suitable for real-time display
  - `false`: Returns the full response at once, suitable for batch processing
</ParamField>

<ParamField body="max_output_tokens" type="integer">
  Maximum number of tokens to generate in the reply. If exceeded, response `status` will be `"incomplete"` with `incomplete_details.reason` as `"max_output_tokens"`.
</ParamField>

<ParamField body="temperature" type="number">
  Sampling temperature, range `0` ~ `2`. Higher values increase randomness, lower values increase determinism. It is not recommended to modify both `temperature` and `top_p` simultaneously.
</ParamField>

<ParamField body="top_p" type="number">
  Nucleus sampling probability, range `0` ~ `1`. The model samples tokens only from the subset whose cumulative probability reaches `top_p`.
</ParamField>

<ParamField body="tools" type="array">
  List of tools that the model can invoke. Currently only supports `type: "function"`, does not support built-in tools like `web_search`, `file_search`, `computer_use`, etc.
  ...
  <Expandable title="Function Tool Format">
    ...
    <ParamField body="description" type="string">
      Function description to help the model determine when to call this tool
    </ParamField>
...
<ParamField body="tool_choice" type="string | object" default="auto">
  ...
  - `"auto"`: model decides automatically whether to invoke tools
  - `"none"`: disallow tool invocation
  - `"required"`: force invocation of at least one tool
  - `{ "type": "function", "name": "function_name" }`: force invoke specific tool
</ParamField>

<ParamField body="parallel_tool_calls" type="boolean" default="true">
  Whether to allow the model to invoke multiple tools in parallel. Effective only when `tools` are provided.
</ParamField>
    ...
    <ParamField body="format" type="object">
      <Expandable title="Format Object">
        <ParamField body="type" type="string" required>
          ...
          - `"json_schema"`: JSON conforming to specified Schema
        </ParamField>
