...
<ParamField body="model" type="string" required>
  The name of the model to use. Examples: `gpt-4o`, `gpt-5.4`, etc.
...
<ParamField body="input" type="array" required>
  ...
  An array of inputs, each containing `role` and `content` fields. Supports multi-turn conversations and multimodal content (text + images).

  <Expandable title="Detailed Field Description">
    <ParamField body="role" type="string" required default="user">
      Message role

      Possible values: `user` (user message), `assistant` (AI reply, for multi-turn conversations), `system` (system prompts)
    </ParamField>

    <ParamField body="content" type="array" required>
      Array of contents

      Supports various types of content blocks, which can include text and images.
      ...
      <Expandable title="Content Block Types">
        ...
        <ParamField body="image_url" type="string">
          ...
          * Note: Must include the `data:image/jpeg;base64,` prefix
        </ParamField>
      </Expandable>
    </ParamField>
  </Expandable>
...
<ParamField body="instructions" type="string">
  System prompt. Used to set the model's behavior guidelines, role identity, or context background.

  Equivalent to a message with `role: "system"` in the message array.
</ParamField>

<ParamField body="stream" type="boolean" default="true">
  Whether to enable streaming output.

  - `true` (default): returns token by token via SSE event stream, suitable for real-time display
  - `false`: waits for the complete response and returns at once, suitable for batch processing
</ParamField>
...
<ParamField body="temperature" type="number">
  Sampling temperature, range `0` to `2`. Higher values produce more random output, lower values more deterministic output. It is not recommended to modify both `temperature` and `top_p` simultaneously.
</ParamField>
...
<ParamField body="tools" type="array">
  List of tools available for the model to call. Currently only supports `type: "function"` type; built-in tools like `web_search`, `file_search`, `computer_use` are not supported.

  <Expandable title="Format of function tools">
    <ParamField body="type" type="string" required>
      Fixed as `"function"`
    </ParamField>
...
<ParamField body="tool_choice" type="string | object" default="auto">
  ...
  - `"none"`: forbid calling tools
  - `"required"`: force at least one tool call
  - `{ "type": "function", "name": "function_name" }`: force call a specified tool
</ParamField>
    ...
    <ParamField body="format" type="object">
      <Expandable title="format object">
        ...
        <ParamField body="name" type="string">
          Schema name (valid only for `json_schema`)
        </ParamField>

        <ParamField body="description" type="string">
          Schema description (valid only for `json_schema`)
        </ParamField>

        <ParamField body="strict" type="boolean" default="true">
          Whether to strictly follow the Schema (valid only for `json_schema`)
        </ParamField>
...
<ParamField body="reasoning" type="object">
  Reasoning mode configuration (applicable for models supporting reasoning, such as `gpt-5.2` and above).
