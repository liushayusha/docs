...
description: 'A unified AI text, image, and video generation API platform. Supports multiple mainstream models, including Claude, GPT, Gemini, VEO3, Seedance2, and more. Compatible with the OpenAI SDK, asynchronous task processing, transparent pricing, simple and easy to use.'
---
import { apiKeyUrl } from '/snippets/_variables.mdx';

AIReiter is a unified AI content generation API platform that provides text conversation, image generation, and video generation services. With simple REST API calls, you can use a variety of mainstream AI models such as Claude, GPT, Gemini, VEO3, and Seedance2. The text interface is fully compatible with the OpenAI SDK and can be used by directly replacing `baseURL`.

## Quick Start

<CardGroup cols={2}>
  <Card title="Text Chat" icon="message" href="/en/api-reference/text/openai-chat-completions-api/endpoint">
    Compatible with the OpenAI SDK, supports Claude, GPT, and other models
  </Card>
  ...
  <Card title="Video Generation" icon="video" href="/en/api-reference/videos/veo3-1/generation">
    Generate video content using models such as VEO3 and Seedance2
  </Card>
  ...
  <Card title="Check Balance" icon="wallet" href="/en/api-reference/tasks/balance">
    Check your token balance and the remaining free quota for each model
  </Card>
...
Model channels represent different service tiers. Each model can provide multiple channel versions, divided into four levels:
...
<Note>
The channel only affects service stability, not output quality. All channels use the same underlying model, and the quality of the generated results is exactly the same. Generally speaking, the higher the channel price, the better the service stability.
</Note>

When using it, just replace the model name with the corresponding channel version. For example, replace `"seedance2"` with `"seedance2_fast"`, and keep the other parameters unchanged.

## Supported Models

### Text Conversation Series

Compatible with the OpenAI Chat Completions API and Responses API, supporting multi-turn conversations, tool calling, structured output, and streaming responses:

<CardGroup cols={2}>
  <Card title="OpenAI Chat Completions API" icon="message" href="/en/api-reference/text/openai-chat-completions-api/endpoint">
    Compatible with the OpenAI SDK, supports models such as GPT and Claude
  </Card>
...
AIReiter provides a variety of image generation models, supporting text-to-image and image-to-image features:
...
<CardGroup cols={2}>
  ...
  <Card title="Nano Banana V2" icon="image" href="/en/api-reference/images/nano-banana-v2/generation">
    Supports multiple generation modes such as text-to-image, image-to-image, and image editing
  </Card>
  <Card title="Seedream-4.0" icon="image" href="/en/api-reference/images/seedream-4/generation">
    An efficient image generation solution
  </Card>
  ...
  <Card title="Seedream V5 lite" icon="image" href="/en/api-reference/images/seedream-v5-lite/generation">
    The latest Seedream version, supports enhanced generation with web search
  </Card>
</CardGroup>

### Video Generation Series

Supports text-to-video and image-to-video generation, providing a variety of resolution and duration options:
...
<CardGroup cols={2}>
  ...
  <Card title="Seedance Pro" icon="video" href="/en/api-reference/videos/seedance_pro/generation">
    Seedance professional version, supports first-and-last frame control
  </Card>
  <Card title="Seedance 2.0" icon="video" href="/en/api-reference/videos/seedance2/generation">
    Supports multiple reference images, reference videos, and first-and-last frame control (4-15 seconds)
  </Card>
  <Card title="Seedance 2.0 Fast" icon="video" href="/en/api-reference/videos/seedance2_fast/generation">
    Seedance 2.0 fast version, supports multiple reference images and first-and-last frame control
  </Card>
</CardGroup>
...
All generation requests use asynchronous processing to avoid long waits:

- When submitting a task, you must pass in `out_task_id` (the initiator's task ID, used for business identification)
- Immediately returns the `out_task_id` you submitted
...
A unified API endpoint that supports multiple text, image, and video generation models:

- **Text Conversation**: Models such as Claude and GPT, compatible with the OpenAI SDK
- **Image Generation**: Models such as GPT-4o, Gemini, and Seedream
- **Video Generation**: Models such as VEO3 and Seedance
- Unified request format and response structure
- Flexible parameter configuration

### Transparent Pricing

Billed by credits, with clear and transparent pricing:

- Each task returns the estimated credit consumption
- The actual credit consumption is shown after the task is completed
...
- **Video**: Multiple aspect ratios (16:9, 9:16), durations (10 seconds, 15 seconds), and resolutions
...
<CardGroup cols={2}>
  <Card title="View Text Chat Documentation" icon="message" href="/en/api-reference/text/openai-chat-completions-api/endpoint">
    Learn how to use the text chat API
  </Card>
  ...
  <Card title="View Video Generation Documentation" icon="video" href="/en/api-reference/videos/veo3-1/generation">
