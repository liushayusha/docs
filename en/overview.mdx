...
<AccordionGroup>
  <Accordion title="How to get the API Key?">
    Visit the <a href={apiKeyUrl} target="_blank">API Key Management page</a>, and log in to create and manage your API Key.
  </Accordion>

  <Accordion title="How long does image and video generation take?">
    All generation tasks are processed asynchronously. Image generation usually takes a few seconds to tens of seconds. Video generation depends on duration and resolution and typically takes 1-5 minutes. After submitting the task, a task_id is returned immediately, and you can track progress via the status query API.
  </Accordion>

  <Accordion title="How to check task status?">
    Use the task status query API by providing the `task_id` returned when submitting the task, or your own `out_task_id` (choose one):
  ...
  <Accordion title="What is out_task_id?">
    `out_task_id` is the initiator's task ID, which you define when submitting the task:

    - **Required Parameter**: You must provide `out_task_id` when submitting the task
    - **Business Identifier**: You can use your system’s order number, business ID, etc. as the identifier
    - **Flexible Query**: You can query task status directly with `out_task_id` without saving the system-returned `task_id`
    - **Result Association**: Both `task_id` and `out_task_id` will be returned in the query result to help associate business data
  </Accordion>

  <Accordion title="Is webhook callback supported?">
    Yes, you can provide the `callback_url` parameter when submitting the task. After completion, the system will send a POST request to that URL, including the full task result.
  </Accordion>

  <Accordion title="How is billing handled?">
    AIReiter uses a credit-based billing system:

    - Estimated credits consumed (`estimated_credits`) are returned when submitting the task
    - Actual credits used (`credits_used`) are returned after the task completes
